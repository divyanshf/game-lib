...
Snake::Snake(SDL_Renderer* ren, SDL_Window* win, std::string username) {
	...
	instructionFont = TTF_OpenFont("Resources/Fonts/Montserrat/MontserratLight-ywBvq.ttf", 18);

	//	Snake
	sShape.setImage("Resources/Assets/Snake/snake.png", ren);
	sShape.setSource(0, 0, 25, 25);
	direction = sDir::STOP;

	//	Fruit
	fruit.setImage("Resources/Assets/Snake/fruit.png", ren);
	fruit.setSource(0, 0, 25, 25);

	//	BestScore
	std::string scoreFileName = "Resources/User/" + username + "/Snake.txt";
	...
	if (!iScoreFile) {
		...
		oScoreFile.close();
		if (_chmod(scoreFileName.c_str(), _S_IREAD) == -1) {
			std::cout << "Could not change the permission mode after creating" << std::endl;
		}
	}
	else {
		std::string tmpScore;
	...
	for (int i = 0; i < nTail; i++) {
		if (i == 0) {
			Object headShape;
			headShape.setImage("Resources/Assets/Snake/head.png", ren);
...
void Snake::logic() {
	...
	if (posX == fruitX && posY == fruitY) {
		...
		if (score > bestScore) {
			...
			std::string scoreFileName = "Resources/User/" + username + "/Snake.txt";
			if (_chmod(scoreFileName.c_str(), _S_IREAD | _S_IWRITE) == -1) {
				std::cout << "Could not change the permission mode before updating" << std::endl;
			}
			oScoreFile.open(scoreFileName.c_str());
			...
			if (_chmod(scoreFileName.c_str(), _S_IREAD) == -1) {
				std::cout << "Could not change the permission mode after updating" << std::endl;
			}
		}
